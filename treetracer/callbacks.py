...
def register_callbacks(app,dataframes):
    ...
    def update_uploaded_files(contents, filenames, dates, stored_data):
        ...
        for i, (content, filename, date) in enumerate(zip(contents, filenames, dates)):
            ...
            if not filename.endswith('.tsv'):
                error_message = f"Only TSV files are allowed. '{filename}' was rejected."
                continue
            ...
            if item['filename'] in selected_files:
                ...
                if df is not None:
                    file_info.append(
                        dmc.Paper(
                            children=[
                                dmc.Text(f"Filename: {item['filename']}",),
                                dmc.Text(f"Number of Dimensions: {len(item['dimensions'])}"),
                                dmc.Text(f"Dimensions: {item['dimensions']}"),
                                dmc.Text(f"Number of Groups: {len(item['groups'])}"),
                                dmc.Text("Groups: " + ", ".join(item['groups'])),
                                dmc.Text(f"Tree Range: {item['MIN_TREENUM']}-{item['MAX_TREENUM']}"),
    ...
    def clear_uploads(n_clicks):
        if n_clicks:
            ...
            dataframes.clear()
            return json.dumps([]), html.Div("Files cleared."), html.Div("Files cleared.")
    ...
    def display_plots(selected_files, stored_data):
        ...
        DF_TO_PLOT['GROUP_COLORS'] = px.colors.qualitative.Dark24[:len(DF_TO_PLOT['GROUPS'])]
        DF_TO_PLOT['COLOR_DICT'] = {g:c for g,c in zip(DF_TO_PLOT['GROUPS'],
                                                    DF_TO_PLOT['GROUP_COLORS'])}

        add_trace_multiplot(default_fig,
            DF_TO_PLOT['df'],
            x,y,z,
            DF_TO_PLOT['GROUPS'],
    ...
    # Controls row with slider and checkbox
        controls = html.Div([
            ...
            html.Div([
                html.Label('Dimensions:', style={'display': 'block', 'margin-bottom': '2px'}),
                dcc.Checklist(options=mdscols,value=mdscols[:3],id = 'dimensions-box',inline=True),
            ...
            html.Div([
                ...
                dcc.RangeSlider(
                    ...
                    marks={MIN_TREENUM: str(MIN_TREENUM), MAX_TREENUM: str(MAX_TREENUM)},
                    step=1
                )
        ...
        plot_div.append(html.Div(
                            # 3D plot
                            dcc.Graph(figure=default_fig, id='graph',
                                     style={'height': '75vh'}), 
                            style={'width': '95%', 
    ...
    def update_graph(mds_selected, treenum_range, current_figure):
        ...
        df = DF_TO_PLOT['df']
        filtered_dff = df[(df['treenum'] >= treenum_range[0]) & 
                           (df['treenum'] <= treenum_range[1])]
        
        x, y, z = mds_selected

        fig = make_plot_grid()
        add_trace_multiplot(fig,filtered_dff,x,y,z,DF_TO_PLOT['GROUPS'],DF_TO_PLOT['COLOR_DICT'])

         # If we have a current figure, try to preserve visibility settings
        if current_figure and 'data' in current_figure and len(current_figure['data']) == len(fig.data):
